...
export const KEYS: readonly { key: string; what: string }[] = [
  ...
  { key: '産業設置具＋右クリック', what: '足もとの資源を調べ、足りていれば一次産業を建てる' },
  { key: 'N', what: '路線表 — 停留所を路線に並べる（並べるまで荷は 1 個も動かない）' },
  ...
  const sections: HelpSection[] = [
    ...
    },
    {
      ...
      notes: [
        ...
        `線路の路線は全区間が同じ速さ（${RAIL_QUALITY.toFixed(2)} 倍・呼び名は「${roadGrade(RAIL_QUALITY)}」）で、列車は一度に ${TRAIN_LOAD} 倍運ぶ。舗装も幅も勾配も関係しない。`,
    ...
    },
    {
      ...
      notes: [
        ...
        '**車両は選ばない。** 区間ごとに、実際に何がつないでいるかで決まる — 線路なら列車、幅 3 マスの道なら荷車、それ以外は徒歩。',
    ...
    },
    {
      ...
      notes: [
        ...
        '発展した町は空いた区画に建物を建てるが、道が通っている区画には建てない。敷いた道はそのまま残る。',
      ],
